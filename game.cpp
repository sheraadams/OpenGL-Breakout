...
void Game::SpawnPowerUps(GameObject &block)
{
    ...
    if (ShouldSpawn(5))
        this->PowerUps.push_back(PowerUp("sticky", glm::vec3(0.0f, 0.5f, 0.0f), 20.0f, block.Position, ResourceManager::GetTexture("powerup_sticky"))); // pear
    if (ShouldSpawn(5))
        this->PowerUps.push_back(PowerUp("pass-through", glm::vec3(1.0f, .50f, 0.0f), 10.0f, block.Position, ResourceManager::GetTexture("powerup_passthrough"))); // orange
    if (ShouldSpawn(5))
        this->PowerUps.push_back(PowerUp("pad-size-increase", glm::vec3(1.5f, 1.5f, 0.0), 0.0f, block.Position, ResourceManager::GetTexture("powerup_increase"))); // lemon
    if (ShouldSpawn(5)) // Negative powerups should spawn more often
        this->PowerUps.push_back(PowerUp("confuse", glm::vec3(0.64f, 0.0f, 1.0f), 15.0f, block.Position, ResourceManager::GetTexture("powerup_confuse"))); // grape
    if (ShouldSpawn(5))
        this->PowerUps.push_back(PowerUp("chaos", glm::vec3(0.9f, 0.0f, 0.0f), 15.0f, block.Position, ResourceManager::GetTexture("powerup_chaos"))); // cherry
}
...
void ActivatePowerUp(PowerUp &powerUp)
{
    ...
    {
        Player->Size.y -= 30;
        Player->Size.x -= 30;  // fixme todo here
    } 
    else if (powerUp.Type == "confuse")
    {
        if (!Effects->Chaos)
...
void Game::DoCollisions()
{
    ...
    if (!Ball->Stuck && std::get<0>(result))
    {
        ...
        // if Sticky powerup is activated, also stick ball to fox once new velocity vectors were calculated
